...
class MorningAgentApp(rumps.App):
    def __init__(self) -> None:
        ...
        self.menu = [
            ...
            rumps.MenuItem("Send Email Now", callback=self.send_now),
            None,
            rumps.MenuItem("Settings…", callback=self.open_settings),
            self.autosend_item,
            rumps.MenuItem("Open config file", callback=self.open_config),
            rumps.MenuItem("About Morning Agent", callback=self.about),
            None,
        ]
    ...
    def send_now(self, _=None) -> None:
        """Send the brief by email right now (regardless of the scheduled mode)."""
        self._deliver("email", mark_state=False)
